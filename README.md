...
The home page supports filtering games by one or more categories and by publisher. Filters run in the browser against the pre-rendered catalog, and the selected values are stored in URL query parameters so a filtered view can be shared or restored.
...
Repository-wide coding standards are maintained in [`.github/copilot-instructions.md`](.github/copilot-instructions.md) and the focused [instruction files](.github/instructions/). In particular, comments should explain intent and decisions rather than restating code; exported data-layer functions must document their purpose, parameters, and return values with TSDoc/JSDoc; and reusable Astro components must document their `Props` contract. TypeScript formatting and explicit module-boundary types are checked by ESLint.
